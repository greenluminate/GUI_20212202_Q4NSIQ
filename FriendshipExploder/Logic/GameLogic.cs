    ...
    public class GameLogic : IGameModel, IGameControl
    {
        ...
        public void SetupRectSize(int gameRectSize)
        {
            ...
            lock (_PlayersListLockObject)
            {
                foreach (var pl in PlayersStore)
                {
                    ...
                    Players.Where(p => p.Id == pl.Id).FirstOrDefault().Position = position;
                }
            }
            //}
        }
        ...
        private async void AIWakeUp(Player ai)
        {
            try
            {
                ...
                while (Players.Where(p => p.KeyBinding == Model.KeyBinding.ai).ToList().All(ai => ai.Position == new Point(0, 0)))
                {
                    Thread.Sleep(500);
                }
